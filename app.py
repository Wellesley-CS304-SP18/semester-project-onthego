...
def admin():
	if (request.method == 'GET'):
		conn = dbconn2.connect(dsn)
		orders = functions.getActiveOrders(conn)
		names = {}
		times = {}
		for item in orders:
			names[item['mid']] = functions.getName(conn, item['mid'])
			times[item['pid']] = functions.getTime(conn, item['pid'])
		return render_template('admin.html',title="Admin View", orders = orders, names=names, times=times)
	else:
		conn = dbconn2.connect(dsn)
		pid = request.form['pid']
		functions.markAsComplete(conn, pid)
		orders = functions.getActiveOrders(conn)
		names = {}
		times = {}
		for item in orders:
