...
# route for the page to place an order
...
# route for student admins who will fill orders
...
def distribute(): 
	if (request.method == 'GET'):
		...
		org = functions.getOrg(adminID, conn)
		activeOrders = functions.getActiveOrders(adminID, conn)

		return render_template('admin.html', title="Orders to " + org, orders=activeOrders)
